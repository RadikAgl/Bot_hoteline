...
def get_searching_commands(message: Message) -> None:
    ...
    """
    logger.info("\n" + "=" * 100 + "\n")
    redis_db.set(gen_key(message, 'state'), 1)
    if 'lowprice' in message.text:
        redis_db.set(gen_key(message, 'order'), 'PRICE')
        logger.info('"lowprice" command is called')
    elif 'highprice' in message.text:
        redis_db.set(gen_key(message, 'order'), 'PRICE_HIGHEST_FIRST')
        logger.info('"highprice" command is called')
    else:
        redis_db.set(gen_key(message, 'order'), 'DISTANCE_FROM_LANDMARK')
        logger.info('"bestdeal" command is called')
    logger.info(redis_db.get(gen_key(message, 'order')))
    state = redis_db.get(gen_key(message, 'state'))
    logger.info(f"Current state: {state}")
    bot.send_message(message.chat.id, make_message(message, 'question_'))
...
def get_command_help(message: Message) -> None:
    ...
    if 'start' in message.text:
        ...
        if lang != 'ru':
            lang = 'en'
        redis_db.set(gen_key(message, 'state'), 0)
        redis_db.set(gen_key(message, 'language'), lang)
        redis_db.set(gen_key(message, 'locale'), locales[lang])
        redis_db.set(gen_key(message, 'currency'), currencies[lang])
...
def keyboard_handler(call: CallbackQuery) -> None:
    ...
    if call.data.startswith('code'):
        ...
        redis_db.set(gen_key(call.message, 'destination_id'), call.data[4:])
        redis_db.set(gen_key(call.message, 'destination_name'), loc_name)
        logger.info(f"{loc_name} selected")
        bot.send_message(
            call.message.chat.id,
            f"{_('loc_selected', call.message)}: {loc_name}",
        )
        if redis_db.get(gen_key(call.message, 'order')) == 'DISTANCE_FROM_LANDMARK':
            redis_db.incr(gen_key(call.message, 'state'), 1)
        else:
            redis_db.incr(gen_key(call.message, 'state'), 3)
        bot.send_message(call.message.chat.id, make_message(call.message, 'question_'))

    elif call.data.startswith('set'):
        redis_db.set(gen_key(call.message, 'state'), 0)
        menu = telebot.types.InlineKeyboardMarkup()
        if call.data == 'set_locale':
            logger.info(f'меню смены языка')
            ...
            logger.info(f'меню смены валюты')
        ...
        bot.send_message(call.message.chat.id, _('ask_to_select', call.message), reply_markup=menu)

    elif call.data.startswith('loc'):
        redis_db.set(gen_key(call.message, 'locale'), call.data[4:])
        redis_db.set(gen_key(call.message, 'language'), call.data[4:6])
        bot.send_message(call.message.chat.id, f"{_('current_language', call.message)}: {_('language', call.message)}")
        logger.info(f"Language changed to {redis_db.get(gen_key(call.message, 'language'))}")
        logger.info(f"Locale changed to {redis_db.get(gen_key(call.message, 'locale'))}")

    elif call.data.startswith('cur'):
        redis_db.set(gen_key(call.message, 'currency'), call.data[4:])
        bot.send_message(call.message.chat.id, f"{_('current_currency', call.message)}: {call.data[4:]}")
        logger.info(f"Currency changed to {redis_db.get(gen_key(call.message, 'currency'))}")

    elif call.data == 'cancel':
        logger.info(f'Canceled by user')
        redis_db.set(gen_key(call.message, 'state'), 0)
        bot.send_message(call.message.chat.id, _('canceled', call.message))
...
def get_search_parameters(msg: Message) -> None:
    ...
    if not is_input_correct(msg):
        ...
        redis_db.incr(gen_key(msg, 'state'))
        if state == '2':
            min_price, max_price = sorted(msg.text.strip().split(), key=int)
            redis_db.set(gen_key(msg, steps[state + 'min']), min_price)
            logger.info(f"{steps[state + 'min']} set to {min_price}")
            redis_db.set(gen_key(msg, steps[state + 'max']), max_price)
            logger.info(f"{steps[state + 'max']} set to {max_price}")
            bot.send_message(msg.chat.id, make_message(msg, 'question_'))
        elif state == '4':
            redis_db.set(gen_key(msg, steps[state]), msg.text.strip())
            logger.info(f"{steps[state]} set to {msg.text.strip()}")
            redis_db.set(gen_key(msg, 'state'), 0)
            hotels_list(msg)
        else:
            redis_db.set(gen_key(msg, steps[state]), msg.text.strip())
            logger.info(f"{steps[state]} set to {msg.text.strip()}")
...
def hotels_list(msg: Message) -> None:
    ...
    if not hotels or len(hotels) < 1:
        bot.send_message(msg.chat.id, _('hotels_not_found', msg))
    elif 'bad_request' in hotels:
        bot.send_message(msg.chat.id, _('bad_request', msg))
    else:
        quantity = len(hotels)
        bot.send_message(msg.chat.id, get_parameters_information(msg))
        bot.send_message(msg.chat.id, f"{_('hotels_found', msg)}: {quantity}")
        for hotel in hotels:
            bot.send_message(msg.chat.id, hotel)
